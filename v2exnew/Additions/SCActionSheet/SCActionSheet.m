...
- (instancetype)initWithTitles:(NSArray *)titles customViews:(NSArray *)customViews buttonTitles:(NSArray *)buttonTitles {
    ...
    if (self = [super initWithFrame:[UIScreen mainScreen].bounds]) {
        ...
        self.contentView = [[UIView alloc] initWithFrame:(CGRect){0, 0, kScreenHeight, 0}];
        ...
        if (titles.count) {
            ...
            if (customViews.count == 0) {
                ...
                for (NSInteger i = 0; i < titles.count; i ++) {
                    ...
                    UIView *lineView;
                    if (i < titles.count - 1 || buttonTitles.count) {
...
- (void)sc_hide:(BOOL)animated {
    ...
    if (animated) {
        [UIView animateWithDuration:kAnimationDuration + (self.contentHeight / 120) * 0.1 delay:0.0 usingSpringWithDamping:1 initialSpringVelocity:1.2 options:UIViewAnimationOptionCurveLinear animations:^{
            self.backgroundButton.alpha = 0.0f;
            self.backgroundMaskView.y = kScreenHeight;
            self.blurBackgroundImageView.y = - kScreenHeight;
            self.contentView.y = kScreenHeight;
