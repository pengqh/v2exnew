...
@interface QHTopicReplyCell () <TTTAttributedLabelDelegate, IDMPhotoBrowserDelegate>
...
@implementation QHTopicReplyCell
...
- (void)setModel:(QHReplyModel *)model {
    _model = model;
...
- (void)layoutContent {
    ...
    @weakify(self);
    [self.model.contentArray enumerateObjectsUsingBlock:^(QHContentBaseModel *baseModel, NSUInteger idx, BOOL *stop) {
        @strongify(self);
        
        if (baseModel.contentType == V2ContentTypeString) {
            QHContentStringModel *stringModel = (QHContentStringModel *)baseModel;
        ...
        if (baseModel.contentType == V2ContentTypeImage) {
            QHContentImageModel *imageModel = (QHContentImageModel *)baseModel;
...
- (void)attributedLabel:(TTTAttributedLabel *)label didSelectLinkWithURL:(NSURL *)url {
    ...
    if (quote) {
        ...
        if (quote.type == SCQuoteTypeUser) {
            ...
            QHReplyModel *replyModel;
            for (QHReplyModel *model in self.replyList.list) {
...
//            [self.navi pushViewController:profileVC animated:YES];
        ...
        if (quote.type == SCQuoteTypeTopic) {
            ...
            QHTopicViewController *topicVC = [[QHTopicViewController alloc] init];
...
+ (CGFloat)getCellHeightWithReplyModel:(QHReplyModel *)model {
    ...
    if (model.contentArray) {
        ...
        [model.contentArray enumerateObjectsUsingBlock:^(QHContentBaseModel *contentModel, NSUInteger idx, BOOL *stop) {
            
            if (contentModel.contentType == V2ContentTypeString) {
                
                QHContentStringModel *stringModel = (QHContentStringModel *)contentModel;
            ...
            if (contentModel.contentType == V2ContentTypeImage) {
                ...
                QHContentImageModel *imageModel = (QHContentImageModel *)contentModel;
                CGSize imageSize = [[self class] imageSizeForKey:imageModel.imageQuote.identifier];
